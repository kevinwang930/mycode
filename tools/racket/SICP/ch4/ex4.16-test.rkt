#lang racket
(require rackunit "ex4.16.rkt")

(test-equal? "number" 1 (interpret 1))
(test-equal? "quoted symbol" 'hello (interpret '(quote hello)))
(test-case "frame"
           (test-equal? "frame" 1 (binding-value (first-binding (make-frame (list 'a)
                                                                   (list 1)))))
           )
(test-case "define"
 (test-equal? "definition 1" 1 (interpret '(let ((a 1))
