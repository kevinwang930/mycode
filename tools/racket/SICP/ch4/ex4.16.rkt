...
(define (apply# procedure arguments)
  (cond
    ...
    ((compound-procedure? procedure)
     (eval-sequence (procedure-body-seqs procedure)
                    (extend-environment
...
(define (empty-frame? frame) (null? (frame-bindings frame)))
(define (make-frame variables values)
  (let ((frame (the-empty-frame)))
    (if (null? variables)
        frame
        (add-to-frame! variables values frame))))
(define (add-to-frame! vars vals frame)
  (if (null? vars)
      frame
      (let ((first-binding (mcons (car vars) (car vals))))
        (add-binding-to-frame! first-binding frame)
        (add-to-frame! (cdr vars) (cdr vals) frame))))
...
(define (binding-variable b) (mcar b))
(define (binding-value b) (mcdr b))
(define (set-binding-value! b val) (set-mcdr! b val))
(define (add-binding-to-frame! binding frame)
  (set-mcdr! frame (mcons binding (mcdr frame))))
...
(define (lookup-variable-value var env)
  (define (error-handle result)
    (if (eq? result '*unassigned*)
        (error "binding unassigned" var)
        result))
  (define (scan var mframe)
    (let ((binding (assoc var (frame-bindings mframe))))
      (if binding
          (error-handle (binding-value binding))
...
(define (scan-out-defines body-seqs)
  (let ((define-clauses-list '())
        (assignment-list '()))
      ...
      (if (null? body)
          ...
          (let ((first (first-exp body)))
            (if (tagged-list? first 'define)
                (begin
                  (set! define-clauses-list (cons
                                             (list (definition-variable first)
                                                   (list 'quote '*unassigned*)
                                                   )
                                             define-clauses-list))
                  (set! assignment-list
                        (cons
                         (make-assignment (definition-variable first)
                                          (definition-value first))
                         assignment-list))
                  (scan-body (rest-exps body)))
                (cons first
                      (scan-body (rest-exps body)))))))
    (let ((scan (scan-body body-seqs)))
      (if (null? define-clauses-list)
          body-seqs
          (make-seqs (make-let (reverse define-clauses-list)
                               (append (reverse assignment-list)
                                       scan)))))))

;lambda expressions
(define (lambda? exp) (tagged-list? exp 'lambda))
(define (lambda-parameters exp) (cadr exp))
(define (lambda-body-seqs exp) (cddr exp))
(define (make-lambda parameters body-sequences)
  (cons 'lambda (cons parameters body-sequences)))
(define (eval-lambda exp env)
  (make-procedure (lambda-parameters exp)
                  (lambda-body-seqs exp)
                  env))
...
(define (make-seqs . exp) exp)
(define (make-procedure parameters body-seqs env)
  (list 'procedure parameters (scan-out-defines body-seqs) env))
...
(define (procedure-parameters p)
  (cadr p))
(define (procedure-body-seqs p) (caddr p))
...
(define (let-body-seqs exp) (cddr exp))
  ...
  (let ((clauses (let-clauses exp)))
    (if (last-clause? clauses)
        ...
        (let ((first-clause (car clauses))
          ...
          (let ((rest-let* (make-let* rest-clauses (let-body-seqs exp))))
